...
public class InputText : MonoBehaviour {
	...
	void AcceptStringInput(string userInput, Noun noun) {
        userInput = userInput.ToLower();
        var conn = new SQLiteConnection(Path.GetDirectoryName(filename));
        var query = conn.Query<Noun>("select phonetics from noun where word = ?", noun.Word);
        phonetic.text = query.ToString();
        InputComplete();
    }
